...
public class Texts : MonoBehaviour
{
    ...
    public int Playerinfest;
    public int NPCinfest;
    ...
    void Update() {
        ...
        if(playerInRange && textEnd == arry.Length && arry[SentenceCount] == dialog.Last() && endDialog == dialog){
            continueText.text = "Press 1 or 2";
    ...
    private void OnTriggerStay2D(Collider2D other){
        ...
        if(addpercent == true){
            other.GetComponent<Healths>().UpdatePlayer(+Playerinfest);
            other.GetComponent<Healths>().UpdateNPC(+NPCinfest);
            addpercent = false;
            Debug.Log("Player still in range");
        }
    }
